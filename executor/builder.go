...
// Internal SQL doesn't use velox. 
func (b *executorBuilder) buildWithInternalSQL(p plannercore.Plan) Executor {
	switch v := p.(type) {
	case nil:
		return nil
...
func (b *executorBuilder) buildWithVelox(p plannercore.Plan) Executor {
	switch v := p.(type) {
	case nil:
		return nil
	case *plannercore.Change:
...
func (b *executorBuilder) buildTableReaderVelox(v *plannercore.PhysicalTableReader) Executor {
	...
	logutil.BgLogger().Info("Velox log buildTableReaderVelox done")
	return b.veloxExec
}
